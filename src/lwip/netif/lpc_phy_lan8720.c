...
#define LAN8_RESTART_AUTONEG (1 << 9)  /* 1=Restart auto-negotiation */
...
#define LAN8_AUTONEG_COMP   (1 << 5)   /* Auto-negotiation complete */
#define LAN8_RMT_FAULT      (1 << 4)   /* Fault */
#define LAN8_AUTONEG_ABILITY (1 << 3)  /* Auto-negotiation supported */
...
/**
 * PHY status structure used to indicate current status of PHY.
 */
typedef struct {
...
/**
 * Last PHY update flags, used for determining if something has changed
 */
 ...
 *
 * This function provides a state machine for maintaining the PHY status without
 * blocking. It must be occasionally called for the PHY status to be maintained.
 *
 * \param [in]     netif   NETIF structure
