...
__attribute__ ((section (".lwip_ram"))) struct lpc_enetdata lpc_enetdata;

/**
 * Sets up the activity LED
 */
void lpc_net_activity_init(void) {
  LPC_SC->PCONP |= (1<<2); /* Power up Timer 1, CCLK/4 */

  LPC_TIM1->TCR = 0x2; /* Put the counter into reset */
  LPC_TIM1->PR = 25000; /* 100ms on a 100MHz clock */
  LPC_TIM1->MR0 = 100;
  LPC_TIM1->MCR |= (1<<0)|(1<<2); /* Interrupt and stop on MR0 */

  NVIC_SetPriority(TIMER1_IRQn, 30); /* Priority is just above the main processing loop */
  NVIC_EnableIRQ(TIMER1_IRQn);
}
/**
 * Turns on the activity LED
 */
void lpc_net_activity(void) {
  LPC_TIM1->TCR = 0x2; /* Put the counter into reset */

  YELLOW_ON();

  LPC_TIM1->TCR = 0x1; /* Start the counter */
}
/**
 * And turns it off again
 */
void TIMER1_IRQHandler(void) {
  LPC_TIM1->IR |= 0x3F; /* Clear all the timer interrupts */
  YELLOW_OFF();
}
 ...
 * EMAC_MIND_SCAN, EMAC_MIND_NOT_VAL, and EMAC_MIND_MII_LINK_FAIL
 */
 ...
 * Queues a pbuf into the RX descriptor list
    ...
    \param lpc_enetdata Pointer to the driver data structure
...
s32_t lpc_rx_queue(struct netif *netif) {
  ...
  /* Exit if there are no descriptors available */
...
static err_t lpc_rx_setup(struct lpc_enetdata *lpc_enetdata) {
  ...
  /* Check the RX structures are aligned to a 8-byte address boundary */
  if ((u32_t)&lpc_enetdata->prxs[0] % 8 != 0) {
    LWIP_DEBUGF(UDP_LPC_EMAC | LWIP_DBG_TRACE,
		("lpc_rx_setup: prxs not aligned to 8-byte address boundary!\n"));
  }
...
/**
 * Determine if the passed address is usable for the ethernet DMA
 * controller.
 *
...
static s32_t lpc_packet_addr_notsafe(void *addr) {
  /* Check for legal address ranges (LPC176x) */
  if (((u32_t) addr >= 0x2007C000) && ((u32_t) addr < 0x20084000)) {
    return 0;
  }
...
/**
 * User call for freeing TX buffers that are complete
...
/**
 * Polls if an available TX descriptor is ready. Can be used to
...
/**
 * Low level output of a packet. Never call this from an
...
static err_t lpc_low_level_output(struct netif *netif, struct pbuf *p) {
  ...
  /* Zero-copy TX buffers may be fragmented across multiple payload
  ...
  if (notdmasafe) {
    ...
    /* use the new buffer for descriptor queueing. The original pbuf will
       be de-allocated outside this driver. */
    p = np;
    dn = 1;
  }
#else
...
static err_t lpc_low_level_output(struct netif *netif, struct pbuf *p) {
  ...
  /* Get next index for transmit descriptor */
  idx++;
  if (idx >= LPC_NUM_BUFF_TXDESCS) {
    idx = 0;
  }
 ...
 * the LPC176x. This is meant to be used when NO_SYS=0. It is not
...
static err_t low_level_init(struct netif *netif) {
  ...
  LPC_EMAC->MCFG = EMAC_MCFG_CLK_SEL(0xF) | EMAC_MCFG_RES_MII;
