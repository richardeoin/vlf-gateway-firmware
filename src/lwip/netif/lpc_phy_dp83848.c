...
#define DP8_RESTART_AUTONEG (1 << 9)  /**< 1=Restart auto-negotiation */
#define DP8_DUPLEX_MODE    (1 << 8)   /**< 1=Full duplex mode */
#define DP8_COLLISION_TEST (1 << 7)   /**< 1=Perform collision test */
...
/** \brief  Last PHY update flags, used for determining if something has changed */
...
/** \brief  Initialize the DP83848 PHY.
    ...
    This function initialises the DP83848 PHY. It will block until
...
err_t lpc_phy_init(struct netif *netif)
{
	...
	/* Set these to 2 so there will always be a change and lpc_update_phy_sts
	 * does something the first time its called */
